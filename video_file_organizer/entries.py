...
from video_file_organizer.database.utils import unsuccessful_vfile_exists, \
    add_unsuccessful_vfile
...
class VideoFileEntry:
    ...
    def _validate_videofile(self):
        if unsuccessful_vfile_exists(self.name, self.hash):
            self.error(
    ...
    def error(self, message, add_to_database=True):
        ...
        if add_to_database:
            add_unsuccessful_vfile(self.name, self.hash, message)
            logger.info(
